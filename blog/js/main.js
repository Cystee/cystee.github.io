document.addEventListener('DOMContentLoaded', function () {
  ...
  const adjustMenu = init => {
    ...
    const hideMenuIndex = window.innerWidth <= 901 || headerContentWidth > $nav.offsetWidth - 120
...
function indexTalk() {
    ...
    ls.forEach((item) => {
      text.push(
        item.content
          ...
          .replace(/\*\*(.*?)\*\___*___/g, '<strong>$1</strong>')
  ...
  if (d) talk(d);
    ...
    fetch("https://say.casecori.top/api/v1/memo?creatorId=1&tag=说说&limit=10")
      .then((res) => res.json())
      .then((data) => {
        // 更改地址和ID
        data = toText(data);
